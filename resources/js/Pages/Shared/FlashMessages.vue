<template>
  <div>
    <div v-if="$page.flash.success && show" class="mt-1 mb-1 flex items-center justify-between bg-indigo-500 rounded max-w-7xl">
      ...
      <button type="button" class="group mr-2 p-2" @click="show = false">
        <svg class="block w-2 h-2 fill-indigo-800 group-hover:fill-white" xmlns="http://www.w3.org/2000/svg" width="235.908" height="235.908" viewBox="278.046 126.846 235.908 235.908"><path d="M506.784 134.017c-9.56-9.56-25.06-9.56-34.62 0L396 210.18l-76.164-76.164c-9.56-9.56-25.06-9.56-34.62 0-9.56 9.56-9.56 25.06 0 34.62L361.38 244.8l-76.164 76.165c-9.56 9.56-9.56 25.06 0 34.62 9.56 9.56 25.06 9.56 34.62 0L396 279.42l76.164 76.165c9.56 9.56 25.06 9.56 34.62 0 9.56-9.56 9.56-25.06 0-34.62L430.62 244.8l76.164-76.163c9.56-9.56 9.56-25.06 0-34.62z" /></svg>
      </button>
    </div>
    <div v-if="($page.flash.error || Object.keys($page.errors).length > 0) && show" class="mt-1 mb-1 flex items-center justify-between bg-red-500 rounded max-w-7xl">
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.errors).length === 1">Existe un error en el formulario.</span>
          <span v-else>Existen {{ Object.keys($page.errors).length }} errores en el formulario.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
